...
import {
  useAdminCreateCustomerTag,
  useAdminUpdateCustomerTag,
} from '@/lib/hooks/admin/useAdminCustomerTags';
import type { CustomerTag, CreateCustomerTagPayload } from '@/lib/types/admin';
...
export default function TagFormModal({ open, onClose, tag }: Props) {
  ...
  const create = useAdminCreateCustomerTag();
  const update = useAdminUpdateCustomerTag();
  ...
  useEffect(() => {
    if (tag) {
      ...
      setName(''); setDescription(''); setColor(COLORS[0]); setIsActive(true);
    }
    setError('');
  }, [tag, open]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError('');
    const payload: CreateCustomerTagPayload = { name, description: description || undefined, color, is_active: isActive };
    try {
      if (tag) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <input type="checkbox" id="tag-active" checked={isActive} onChange={(e) => setIsActive(e.target.checked)}
              className="w-4 h-4 rounded border-gray-300 text-amber-400 focus:ring-amber-200" />
            <label htmlFor="tag-active" className="text-sm text-gray-700">Active</label>
          </div>
